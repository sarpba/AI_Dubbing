...
def _resolve_requested_dtype(requested: str, config_dtype) -> torch.dtype:
    """
    Resolve a save dtype string to the corresponding torch dtype.
    Uses the model config dtype when requested is 'auto'.
    """
    key = requested.lower().strip()
    
    if key in {"auto", "match", "config"}:
        if config_dtype is not None:
            if isinstance(config_dtype, torch.dtype):
                return config_dtype
            config_key = str(config_dtype).lower().replace("torch.", "")
            if config_key in _DTYPE_ALIASES:
                return _DTYPE_ALIASES[config_key]
        # Fall back to float32 when config dtype is missing or unrecognised
        return torch.float32
    
    normalized_key = key.replace("torch.", "")
    if normalized_key not in _DTYPE_ALIASES:
        available = ", ".join(sorted({k for k in _DTYPE_ALIASES.keys() if len(k) <= 6}))
        raise ValueError(f"Unsupported dtype '{requested}'. Available options: {available}, auto")
    
    return _DTYPE_ALIASES[normalized_key]
...
def verify_merge(
    ...
    for key in base_state.keys():
        ...
        if not torch.allclose(base_tensor, merged_tensor, rtol=1e-5, atol=1e-8):
...
def merge_vibevoice_model(
    ...
    target_dtype = _resolve_requested_dtype(save_dtype, base_model.config.torch_dtype)
    dtype_name = str(target_dtype).replace("torch.", "")
    
    try:
        current_dtype = next(base_model.parameters()).dtype
    except StopIteration:
        current_dtype = target_dtype
    if current_dtype != target_dtype:
        logger.info(f"Converting model parameters to {dtype_name} before saving...")
        base_model.to(dtype=target_dtype)
    else:
        logger.info(f"Model already in requested dtype: {dtype_name}")
    
    # Keep configs aligned with saved dtype for downstream loading
    base_model.config.torch_dtype = dtype_name
    generation_config = getattr(base_model, "generation_config", None)
    if generation_config is not None:
        generation_config.torch_dtype = dtype_name
...
Examples:
  ...
  python merge_vibevoice_models.py --base_model_path model --output_path merged --verify_only
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        help="Numeric precision for the saved weights. Options: auto, float32/fp32, bfloat16/bf16, float16/fp16"
    )
    parser.add_argument(
        "--verify_only",
        action="store_true",
