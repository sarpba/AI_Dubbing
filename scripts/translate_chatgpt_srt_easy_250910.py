...
# (Ez a rész változatlan marad, ezért a rövidség kedvéért összecsukom)
# ... A teljes konfigurációs és kulcskezelő kódod itt van ...
...
# <<< MÓDOSÍTÁS: A függvény most visszaadja a teljes, egybefűzött szöveget is.
...
# <<< MÓDOSÍTÁS: Ez az új függvény, ami az "igazítást" végzi fordítás helyett.
def align_and_segment_batch(client, batch_segments, target_lang_srt_data, model, stream, batch_id_str):
    ...
    if not target_lang_context:
        ...
        # Visszaadhatnánk üres stringeket, hogy a folyamat ne álljon le
        return [""] * len(batch_segments)

    # Az új, "igazító" prompt
    system_prompt = (
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.0  # Alacsony hőmérséklet a precíz, kreativitásmentes feladathoz
        )
    ...
    if len(aligned_lines_raw) == len(batch_segments):
        ...
        # A rekurzív felosztás itt is hasznos lehet, ha az LLM-nek túl nagy a kontextus
...
def main(project_name, input_lang, output_lang, auth_key_arg, context, model, stream, allow_sensitive_content):
    ...
    # <<< MÓDOSÍTÁS: A magyar SRT fájl betöltése. Ez a forrása a szövegnek.
    ...
    target_srt_data, _ = load_srt_file(target_srt_filepath) # Itt a teljes szövegre nincs szükség, csak a feldolgozott adatra
    ...
    if not unprocessed_segments: print("Nincs új, feldolgozandó szegmens.")
        ...
        for i, batch in enumerate(batches):
            ...
            # <<< MÓDOSÍTÁS: Az új "igazító" függvény hívása a régi fordító helyett
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Meglévő fordítás (SRT) újra-szegmentálása egy ASR JSON időzítése alapján.',
