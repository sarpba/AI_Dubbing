...
def main(project_name, input_lang, output_lang, auth_key_arg, context, model, stream, allow_sensitive_content, use_openai):
    # --- KLIENS BEÁLLÍTÁSA ---
    client = None
    if use_openai:
        print("OpenAI API használata kiválasztva (`--use-openai`).")
        auth_key = auth_key_arg or load_api_key()
        ...
        if auth_key_arg:
            save_api_key(auth_key_arg)
        
        # Figyelmeztetés, ha a modell neve nem tipikus OpenAI modell
        if 'gpt-' not in model.lower() and 'dall-e' not in model.lower():
             print(f"FIGYELMEZTETÉS: Az OpenAI API-t használja, de a modell neve ('{model}') nem tűnik OpenAI modellnek (pl. 'gpt-4o').")
             print("A szkript megpróbálja a futtatást, de győződjön meg róla, hogy ez egy létező, elérhető modell az OpenAI-nál.")

        client = OpenAI(api_key=auth_key)
        print("OpenAI API kliens sikeresen beállítva.")
    else:
        # Alapértelmezett viselkedés: LM Studio
        print("Alapértelmezett módban a szkript a helyi LM Studiohoz csatlakozik.")
        print("Cím: http://127.0.0.1:1234")
        client = OpenAI(
            base_url="http://127.0.0.1:1234/v1",
            api_key="not-needed"  # Az LM Studio API-hoz nem kötelező kulcs
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-auth_key', required=False, help=(
        'Opcionális OpenAI API kulcs. Az `--use-openai` kapcsolóval együtt szükséges.\n'
        'Ha megadjuk, elmentődik a keyholder.json-ba a későbbi futtatásokhoz.'
    ))
    parser.add_argument('-context', required=False, help='Rövid, általános kontextus a fordításhoz (pl. "sci-fi sorozat").')
    parser.add_argument('-model', default='unsloth/gpt-oss-20b', required=False, help='A használni kívánt modell neve (LM Studio vagy OpenAI). Alapértelmezett: unsloth/gpt-oss-20b')
    parser.add_argument('-stream', action='store_true', help='Bekapcsolja a soronkénti kimenetet (nem valódi stream).')
    parser.add_argument('--use-openai', action='store_true', help='Helyi LM Studio helyett az OpenAI API használata. Szükséges hozzá API kulcs.')
