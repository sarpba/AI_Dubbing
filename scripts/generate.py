...
def get_num_gpus():
    ...
    try:
        result = subprocess.run(['nvidia-smi', '-L'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True, text=True)
        print(result.stdout)
        print(result.stderr)
...
def main():
    ...
    if args.ref_audio:
        ...
        if ref_audio_path.lower().endswith('.mp3'):
            ...
            try:
                print(f"Converting {ref_audio_path} to {converted_audio}")
