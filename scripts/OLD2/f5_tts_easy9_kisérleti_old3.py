...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("infer_batch.log")]
)
...
class F5TTS:
    ...
    def load_vocoder_model(self, vocoder_name, local_path):
        # javított hívás, kulcsparaméterekkel
    ...
    def infer(self, ref_file, ref_text, gen_text, file_wave, remove_silence=False, speed=1.0, nfe_step=32, seed=-1):
        ...
        # determinisztika csak fix seednél
        if seed != -1:
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            if phonetic_converter:
                try:
                    ...
                    logger.info(f"Phonetic ref text: '{original_ref_text}' -> '{phonetic_text}'")
            ...
            if args.seed == -1:
                ...
                for attempt in range(1, args.max_retries + 1):
                    ...
                    try:
                        ...
                        final_tolerance = max(calculated_tolerance, args.min_tolerance)
                        # opcionális felső korlát
                        final_tolerance = min(final_tolerance, int(0.4 * word_count) + 5)
                        ...
                        if dist <= final_tolerance:
                            ...
                            logger.warning(
                                f"Worker {gpu}: Verification FAILED (Attempt {attempt}/{args.max_retries}) -> Distance: {dist}, Tolerance: {final_tolerance} for '{gen_text}'"
                            )
                            ...
                            if args.save_failures:
                                ...
                                if attempt == 1:
                                    ...
                                    info_ref_text = (
                                        f"{original_ref_text} (phonetic: {current_ref_text})"
                                        if phonetic_converter and original_ref_text != current_ref_text else original_ref_text
                                    )
...
def main():
    ...
    if args.model_dir:
        ...
        # TTY nélküli fallback: config default használata, ha létezik
