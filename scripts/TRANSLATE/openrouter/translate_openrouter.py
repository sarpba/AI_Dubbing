...
def build_system_prompt(
    ...
    if override and override.strip():
        ...
        if allow_sensitive:
            ...
            system_prompt = (
                f"You are an expert translator. Translate the numbered list from {input_lang_name} to {output_lang_name}. "
                "Your response MUST be a numbered list with the exact same number of items. Format: `1. [translation]`."
            )
    if context:
        system_prompt += f"\n\nContext: '{context}'"
...
def main(
    ...
    if not segments_to_process:
        ...
        system_prompt_final = build_system_prompt(
            input_lang_resolved, output_lang_resolved, allow_sensitive_content, context, systemprompt
        )
        logger.info(
