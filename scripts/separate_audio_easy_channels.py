...
def is_pure_speech(vocals_np: np.ndarray, non_speech_np: np.ndarray, eps=1e-9,
                   rel_thresh_db=-25.0, abs_thresh=-50.0) -> bool:

#    Egyszerű tiszta-beszéd detektálás Demucs kimenetek alapján.
#    - rel_thresh_db: ha a non_speech RMS legalább ennyivel alacsonyabb (dB) a vocals RMS-nél, tiszta beszédnek tekintjük.
#    - abs_thresh: ha a non_speech RMS globálisan is ez alatt a dBFS alatt van, erős jelzés a tiszta beszédre.
...
#    \"""
    try:
        ...
        # Automatikus tiszta-beszéd felismerés
        auto_non_speech_silence = is_pure_speech(vocals, non_speech)

        if non_speech_silence or auto_non_speech_silence:
            if auto_non_speech_silence and not non_speech_silence:
                print("Automatikus tiszta beszéd detektálva, non_speech sáv csendesítése.")
...
#    \"""
    try:
        ...
        for start in range(0, total_samples, chunk_size_samples):
            ...
            # Automatikus tiszta-beszéd felismerés chunkenként
            auto_non_speech_silence = is_pure_speech(vocals_chunk, non_speech_chunk)

            if non_speech_silence or auto_non_speech_silence:
                if auto_non_speech_silence and not non_speech_silence:
                    print("Automatikus tiszta beszéd detektálva ebben a chunkban, non_speech csendesítése.")
