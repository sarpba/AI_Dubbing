...
# ... (F5TTS, time_to_filename_str, resolve_model_paths_from_dir, interactive_model_selection és a legtöbb segédfüggvény változatlan)
class F5TTS:
...
def parse_arguments():
    # ... (A parse_arguments függvény változatlan)
...
def main_worker(gpu, args, all_chunks, input_wav_path_str, stats, failed_segments_info):
    ...
    try:
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            # --- MÓDOSÍTÁS: A NORMALIZÁLÁS A CIKLUSON BELÜL TÖRTÉNIK ---
            ...
            current_ref_audio_chunk=processed_ref_chunk # Az első kísérlet a feldolgozottal indul
            ...
            if args.seed == -1:
                    ...
                    try:
                        ...
                        if dist <= final_tolerance:
                            ...
                            if args.save_failures:
                                ...
                                # === MÓDOSÍTÁS: ÚJ FÁJLNÉV A HIBÁS GENERÁLÁSNAK ===
                                ...
                                info={}
                                if attempt==1: # Csak az első hibánál mentjük a referenciákat
                            ...
                            if args.double_ref_on_failure and attempt == 1:
                                ...
                                current_ref_audio_chunk=np.concatenate([processed_ref_chunk, processed_ref_chunk]) # A már normalizáltat duplázzuk
                                if args.save_failures:
                ...
                try:
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp_ref:
                        sf.write(tmp_ref.name, current_ref_audio_chunk, sample_rate) # Itt a current_ref_audio_chunk a processed_ref_chunk
...
# ... (process_file_pair és main függvények változatlanok, de az olvashatóság kedvéért itt hagyom)
