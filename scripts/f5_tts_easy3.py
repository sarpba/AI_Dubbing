...
def parse_arguments():
    ...
    # === ÚJ KAPCSOLÓ A HIBÁS GENERÁLÁSOK MENTÉSÉHEZ ===
    parser.add_argument("--save-failures", action="store_true", help="Elmenti a hibásan generált hangokat és a hozzájuk tartozó szövegeket egy 'failed_generations' mappába.")
...
def main_worker(gpu, args, all_chunks, input_wav_path_str, stats):
    ...
    try:
        ...
        if args.seed == -1:
            logger.info(f"Worker {gpu}: Random seed. Initializing verification tools.")
            ...
            if is_huggingface_model:
                if pipeline is None:
                    logger.error("Hugging Face model requested, but `transformers` not installed."); return
                ...
                if whisper is None:
                    logger.error("OpenAI model requested, but `openai-whisper` not installed."); return
        ...
        if normaliser_path.exists():
            try:
                ...
                normaliser = importlib.util.module_from_spec(spec); spec.loader.exec_module(normaliser)
                if hasattr(normaliser, 'normalize'):
                    normalize_fn = normaliser.normalize; logger.info(f"Worker {gpu}: Loaded normalizer '{args.norm}'.")
        ...
        logger.info(f"Worker {gpu}: Initialized '{args.resolved_model_architecture}' model.")
        full_audio_data,sample_rate=sf.read(input_wav_path_str);logger.info(f"Worker {gpu}: Loaded main audio file.")
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            if args.seed == -1:
                ...
                for attempt in range(1, args.max_retries + 1):
                    ...
                    try:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".wav",delete=True) as tmp_ref:
                            sf.write(tmp_ref.name,ref_audio_chunk,sample_rate)
                            f5tts.infer(ref_file=tmp_ref.name,ref_text=ref_text,gen_text=gen_text,file_wave=temp_gen_path,seed=-1,speed=args.speed,nfe_step=args.nfe_step)
                        
                        transcribed_text = run_transcription(temp_gen_path)['text']
                        ...
                        if dist <= final_tolerance:
                            ...
                            # === HIBÁS GENERÁLÁS MENTÉSE, HA A KAPCSOLÓ AKTÍV ===
                            if args.save_failures:
                                ...
                                # Ideiglenes fájl átnevezése a debug mappába
                                debug_gen_audio_path = debug_segment_dir / f"gen_audio_attempt_{attempt}.wav"
                                os.rename(temp_gen_path, debug_gen_audio_path)
                                
                                info = {}
                                if attempt == 1:
                                    # Referencia mentése az első hiba esetén
                                    sf.write(debug_segment_dir / "ref_audio.wav", ref_audio_chunk, sample_rate)
                                    info = {"ref_text": ref_text, "gen_text": gen_text, "failures": []}
                                else:
                                    if info_json_path.exists():
                                        with open(info_json_path, 'r', encoding='utf-8') as f: info = json.load(f)
                                
                                # Aktuális hiba adatainak hozzáadása
                                info.get("failures", []).append({'attempt': attempt, 'transcribed_text': transcribed_text, 'distance': dist, 'allowed_tolerance': final_tolerance})
                                with open(info_json_path, 'w', encoding='utf-8') as f: json.dump(info, f, ensure_ascii=False, indent=2)
                            else:
                                os.remove(temp_gen_path) # Ha a mentés ki van kapcsolva, töröljük

                    except Exception as e:
                        logger.error(f"Worker {gpu}: Error on attempt {attempt}: {e}", exc_info=False)
                ...
                # Közvetlen generálás (fix seed)
                try:
                    with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp_ref:
                        sf.write(tmp_ref.name, ref_audio_chunk, sample_rate)
                        f5tts.infer(ref_file=tmp_ref.name,ref_text=ref_text,gen_text=gen_text,file_wave=str(output_wav_path),seed=args.seed,speed=args.speed,nfe_step=args.nfe_step)
                    stats['successful'] += 1
                except Exception as e:
                    logger.error(f"Worker {gpu}: Direct generation FAILED for segment {start_time}-{end_time}: {e}", exc_info=False)
                    stats['failed'] += 1
...
def main():
    ...
    # Kiegészítjük az args objektumot a könyvtárakkal
    ...
    # === ÚJ KÖNYVTÁR HOZZÁADÁSA AZ ARGUMENTUMOKHOZ ===
    args.failed_generations_dir = str(full_project_path / "failed_generations")

    # Kimeneti könyvtárak létrehozása
    ...
    wav_files = list((full_project_path / cfg_subdirs['separated_audio_speech']).glob('*.wav'))
    json_files = list((full_project_path / cfg_subdirs['translated']).glob('*.json'))
    if not wav_files: logger.error(f"No .wav files in {(full_project_path / cfg_subdirs['separated_audio_speech'])}"); sys.exit(1)
    if len(wav_files) > 1: logger.warning(f"Multiple .wav files found, using {wav_files[0]}")
    if not json_files: logger.error(f"No .json files in {(full_project_path / cfg_subdirs['translated'])}"); sys.exit(1)
    if len(json_files) > 1: logger.warning(f"Multiple .json files found, using {json_files[0]}")
