...
# Import Levenshtein globally
try:
    from Levenshtein import distance as levenshtein_distance
except ImportError:
    print("ERROR: Levenshtein module required. Install with: pip install python-Levenshtein")
    sys.exit(1)

# Constants
DEFAULT_MEL_SPEC_TYPE = "vocos"
DEFAULT_REF_PEAK = 0.95
...
SILENCE_DURATION = 0.4  # seconds between repeated segments
MAX_REF_LENGTH = 11.5  # seconds for doubled reference
...
@dataclass
class TTSConfig:
    mel_spec_type: str = DEFAULT_MEL_SPEC_TYPE
    target_sample_rate: int = target_sample_rate
    hop_length: int = hop_length
    ref_peak: float = DEFAULT_REF_PEAK
    min_audio_length: float = MIN_AUDIO_LENGTH
    silence_duration: float = SILENCE_DURATION
    max_ref_length: float = MAX_REF_LENGTH

class F5TTS:
    def __init__(self, model_cls, model_cfg_dict, ckpt_file, vocab_file, 
                 vocoder_name=DEFAULT_MEL_SPEC_TYPE, ode_method="euler", 
                 use_ema=True, local_path=None, device=None):
        self.config = TTSConfig()
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.load_vocoder_model(vocoder_name, local_path)
        self.load_ema_model(model_cls, model_cfg_dict, ckpt_file, 
                           vocoder_name, vocab_file, ode_method, use_ema)

    def load_vocoder_model(self, vocoder_name, local_path):
        self.vocoder = load_vocoder(vocoder_name, local_path is not None, local_path, self.device)

    def load_ema_model(self, model_cls, model_cfg, ckpt_file, mel_spec_type, 
                      vocab_file, ode_method, use_ema):
        self.ema_model = load_model(model_cls, model_cfg, ckpt_file, mel_spec_type, 
                                   vocab_file, ode_method, use_ema, self.device)

    def export_wav(self, wav, file_wave, remove_silence=False):
        sf.write(file_wave, wav, self.config.target_sample_rate)
        if remove_silence:
            remove_silence_for_generated_wav(file_wave)

    def infer(self, ref_file, ref_text, gen_text, file_wave, remove_silence=False, 
             speed=1.0, nfe_step=32, seed=-1):
        # Validate parameters
        if not (MIN_SPEED <= speed <= MAX_SPEED):
            raise ValueError(f"Invalid speed: {speed}. Must be between {MIN_SPEED} and {MAX_SPEED}")
        if not (MIN_NFE_STEP <= nfe_step <= MAX_NFE_STEP):
            raise ValueError(f"Invalid nfe_step: {nfe_step}. Must be between {MIN_NFE_STEP} and {MAX_NFE_STEP}")
        ...
        wav, sr, spect = infer_process(
            ref_file, ref_text, gen_text, self.ema_model, self.vocoder, 
            self.config.mel_spec_type, show_info=logger.info, progress=tqdm, 
            target_rms=0.1, cross_fade_duration=CROSS_FADE_DURATION, 
            nfe_step=nfe_step, cfg_strength=CFG_STRENGTH, sway_sampling_coef=-1, 
            speed=speed, fix_duration=None, device=self.device
        )
        
        # Extend short audio
        wav = self._extend_short_audio(wav, sr, gen_text)
    ...
    def _extend_short_audio(self, wav: np.ndarray, sr: int, gen_text: str) -> np.ndarray:
        """Extend audio if shorter than minimum length by repeating with silence"""
        current_duration = len(wav) / sr
        
        if current_duration >= self.config.min_audio_length:
            return wav
        
        # Calculate how many times we need to repeat
        repeats_needed = math.ceil(self.config.min_audio_length / current_duration)
        
        # Create silence segment
        silence_samples = int(self.config.silence_duration * sr)
        silence = np.zeros(silence_samples, dtype=wav.dtype)
        
        # Repeat audio and text
        extended_wav = [wav]
        for _ in range(repeats_needed - 1):
            extended_wav.append(silence)
            extended_wav.append(wav)
        
        return np.concatenate(extended_wav)
...
def resolve_model_paths_from_dir(model_dir_path: str) -> Tuple[str, str, str]:
    ...
    logger.info(f"Found model files in '{model_dir}':\n"
                f"  - Checkpoint: {resolved_ckpt_file.name}\n"
                f"  - Vocab: {resolved_vocab_file.name}\n"
                f"  - Config: {resolved_json_file.name}")
...
def parse_arguments():
    ...
    parser.add_argument("--speed", type=float, default=1.0, help=f"A generált hang sebessége ({MIN_SPEED}-{MAX_SPEED}).")
    parser.add_argument("--nfe_step", type=int, default=32, help=f"NFE lépések száma ({MIN_NFE_STEP}-{MAX_NFE_STEP}).")
    ...
    parser.add_argument("--ref-audio-peak", type=float, default=DEFAULT_REF_PEAK, help="A normalizált referencia audio cél csúcsértéke (0.0-1.0).")
...
def calculate_tolerance(word_count: int, factor: float, min_tolerance: int) -> int:
    return max(math.ceil(word_count * factor), min_tolerance)

def _initialize_worker(gpu: int, args) -> Tuple:
    """Initialize worker-specific resources"""
    torch.cuda.set_device(gpu)
    device = f"cuda:{gpu}"
    logger.info(f"Worker {gpu} starting on device {device}")
    
    # Initialize phonetic converter if needed
    ...
    if args.phonetic_ref:
        try:
            ...
            try:
                ...
                logger.error(f"Worker {gpu}: Failed to initialize phonetic transcriber: {e}. Disabling feature.")
                phonetic_converter = None
        except ImportError:
            logger.warning(f"Worker {gpu}: --phonetic-ref specified but 'fonetic.py' not available.")
    ...
    if args.seed == -1:
        ...
        try:
            if is_huggingface_model:
                # Import pipeline inside the try block to ensure it's available
                ...
                transcriber = whisper.load_model(
                    args.whisper_model.replace("openai/", ""),
                    device=device
                )
            ...
            def create_transcription_func(t, is_hf, lang, beam):
                def func(p):
                    if is_hf:
                        k = {
                            "language": WHISPER_LANG_CODE_TO_NAME.get(lang),
                            "num_beams": beam
                        }
                        return t(p, generate_kwargs=k)
                    else:
                        return t.transcribe(
                            p,
                            language=lang,
            ...
            run_transcription = create_transcription_func(
                transcriber, is_huggingface_model, whisper_language, args.beam_size
            )
        except Exception as e:
            logger.error(f"Worker {gpu}: Failed to initialize transcription: {e}")
            run_transcription = None  # Explicitly set to None
    ...
    return device, phonetic_converter, run_transcription, normalize_fn, f5tts

def _process_segment_with_retry(
    segment: Dict,
    device: str,
    f5tts: F5TTS,
    phonetic_converter: Optional[callable],
    run_transcription: Optional[callable],
    normalize_fn: Optional[callable],
    args,
    full_audio_data: np.ndarray,
    sample_rate: int,
    stats: Dict,
    failed_segments_info: List
) -> bool:
    ...
    if not all([isinstance(start_time, (int, float)), 
                isinstance(end_time, (int, float)), 
                original_ref_text, original_gen_text]):
        return False
    ...
    if phonetic_converter:
        try:
            ...
            logger.error(f"Worker {device}: Failed to convert ref text: {e}")
    ...
    # For fixed seed, just generate once
    if args.seed != -1:
        try:
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp_ref:
                sf.write(tmp_ref.name, processed_ref_chunk, sample_rate)
                f5tts.infer(
            ...
            logger.error(f"Worker {device}: Fixed seed generation failed: {e}")
            stats['failed'] += 1
            failed_segments_info.append({'filename': filename, 'text': original_gen_text})
            return False
    
    # For random seed, check if transcription is available
    if run_transcription is None:
        logger.warning(f"Worker {device}: Transcription not available. Using fixed seed generation.")
        try:
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp_ref:
                sf.write(tmp_ref.name, processed_ref_chunk, sample_rate)
                f5tts.infer(
            ...
            logger.error(f"Worker {device}: Random seed generation failed: {e}")
            stats['failed'] += 1
            failed_segments_info.append({'filename': filename, 'text': original_gen_text})
            return False
    
    # For random seed with transcription, use retry logic
    best_attempt = None
    best_distance = float('inf')
    
    # Calculate tolerance
    word_count = len(gen_text.split())
    final_tolerance = calculate_tolerance(word_count, args.tolerance_factor, args.min_tolerance)
    
    current_ref_audio = processed_ref_chunk
    current_ref_text_for_retry = current_ref_text
    ...
    for attempt in range(1, args.max_retries + 1):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
                ...
                temp_files.append(temp_gen_path)  # Register for cleanup
            
            # Generate audio
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp_ref:
                sf.write(tmp_ref.name, current_ref_audio, sample_rate)
                f5tts.infer(
                    ref_file=tmp_ref.name,
                    ref_text=current_ref_text_for_retry,
                    gen_text=gen_text,
            ...
            raw_transcribed_text = run_transcription(temp_gen_path)['text']
            converted_transcribed_text = convert_numbers_to_words(
                raw_transcribed_text, lang=NORMALIZER_TO_WHISPER_LANG.get(args.norm.lower())
            )
            ...
            # Handle special case for Hungarian articles
            ...
            if distance < best_distance:
                ...
                best_attempt = temp_gen_path
                temp_gen_path = None  # Don't delete this one yet
            
            # Double reference on first failure if enabled
            if args.double_ref_on_failure and attempt == 1:
                ref_duration = len(current_ref_audio) / sample_rate
                if ref_duration * 2 <= MAX_REF_LENGTH:
                    logger.info(f"Worker {device}: Doubling reference audio for next attempts")
                    current_ref_audio = np.concatenate([current_ref_audio, current_ref_audio])
                    current_ref_text_for_retry = f"{current_ref_text_for_retry} {current_ref_text_for_retry}"
            
            # Log attempt
            ref_audio_duration = len(current_ref_audio) / sample_rate
            is_phonetic_used = phonetic_converter and original_ref_text != current_ref_text
            
            log_message = (
                f"\n--- Generálás Log (Worker {device}, Fájl: {filename}) ---\n"
                ...
                f"Maximális megengedett és elért távolság: | {distance}/{final_tolerance}\n"
                f"Generálás sikeres: | {'Igen' if distance <= final_tolerance else 'Nem'}\n"
                f"----------------------------------------------------------"
            )
            ...
            if args.save_failures:
                ...
                debug_segment_dir.mkdir(exist_ok=True)
                ...
                if attempt == 1:
                    ...
                    info_ref_text = f"{original_ref_text} (phonetic: {current_ref_text})" if is_phonetic_used else original_ref_text
                    info = {
                        "ref_text": info_ref_text,
                        "gen_text": gen_text,
                        "failures": []
                    }
            ...
            logger.error(f"Worker {device}: Error during attempt {attempt} for '{filename}': {e}")
    ...
    if best_attempt and best_distance <= final_tolerance:
        os.rename(best_attempt, output_wav_path)
        stats['successful'] += 1
        return True
...
def main_worker(gpu: int, args, all_chunks: List, input_wav_path_str: str, stats: Dict, failed_segments_info: List):
    """Main worker function"""
    try:
        # Initialize worker
        device, phonetic_converter, run_transcription, normalize_fn, f5tts = _initialize_worker(gpu, args)
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            _process_segment_with_retry(
                segment, device, f5tts, phonetic_converter, run_transcription,
                normalize_fn, args, full_audio_data, sample_rate, stats, failed_segments_info
            )
    except Exception as e:
        logger.critical(f"Critical error in worker {gpu}: {e}", exc_info=True)

def process_file_pair(args, input_wav_path: Path, input_json_path: Path):
    """Process a single audio/JSON pair"""
    ...
    for segment in segments:
        ...
        if start_time is not None and start_time > last_end_time:
            ...
            if not noise_output_path.exists():
                sf.write(noise_output_path, full_audio_data[int(last_end_time*sample_rate):int(start_time*sample_rate)], sample_rate)
    ...
    with mp.Manager() as manager:
        ...
        logger.info("="*50 + f"\nVÉGSŐ STATISZTIKA\n"
                    f"  - Összes szegmens: {t}\n"
                    f"  - Sikeres generálások: {s}\n"
                    f"  - Sikertelen generálások: {f}\n" + "="*50)
