...
def load_narrator_reference_corpus(narrator_dir: Path) -> Tuple[np.ndarray, int, str, int]:
    ...
    for audio_path in audio_files:
        ...
        if sample_rate is None:
            ...
            raise ValueError(
                f"A narrátor referenciafájlok mintavételezése eltérő ({sr} vs {sample_rate}). "
                "Kérlek, egységesítsd őket."
            )

        audio_segments.append(audio_data.astype(np.float32, copy=False))
        texts.append(text.replace("\n", " ").strip())

    if not audio_segments or sample_rate is None:
        raise RuntimeError("Nem sikerült narrátor referencia hanganyagot betölteni.")

    silence_samples = max(1, int(sample_rate * REF_SILENCE_PADDING_SECONDS))
    silence = np.zeros(silence_samples, dtype=np.float32)

    stitched_audio_parts: List[np.ndarray] = []
    for idx, segment_audio in enumerate(audio_segments):
        stitched_audio_parts.append(segment_audio)
        if idx < len(audio_segments) - 1:
            stitched_audio_parts.append(silence)

    narrator_audio = np.concatenate(stitched_audio_parts)
    narrator_text = " ".join(texts).strip()
    if narrator_audio.size == 0 or not narrator_text:
        raise RuntimeError("A narrátor referencia audio vagy szöveg üres.")

    pair_count = len(audio_segments)
    return narrator_audio.astype(np.float32, copy=False), sample_rate, narrator_text, pair_count
...
def main_worker(gpu, args, all_chunks, stats, failed_segments_info):
    if gpu >= len(all_chunks):
        logger.warning("Worker %s: No task chunk assigned.", gpu)
        return

    tasks_chunk = all_chunks[gpu]
    if not tasks_chunk:
        logger.info("Worker %s: Nothing to process.", gpu)
        return
    ...
    if torch.cuda.is_available():
        ...
        device = "cpu"
    logger.info("Worker %s starting on device %s with %s szegmenssel.", gpu, device, len(tasks_chunk))

    phonetic_converter: Optional[Callable[[str], str]] = None
    if args.phonetic_ref:
        if fonetikus_atiras:
            logger.info("Worker %s: Fonetikus referencia mód engedélyezve.", gpu)
            phonetic_converter = fonetikus_atiras
            try:
                phonetic_converter("initialize")
            except Exception as exc:
                logger.error(
                    "Worker %s: Fonetikus transzformáció inicializálása sikertelen (%s). Funkció kikapcsolva.",
                    gpu,
                    exc,
                    exc_info=True,
                )
            ...
            logger.warning(
                "Worker %s: --phonetic-ref megadva, de a 'fonetic.py' nem érhető el. Funkció kikapcsolva.",
                gpu,
            )
    ...
    try:
        ...
        if args.seed == -1 and transcribe_fn is None:
            logger.error("Worker %s: Verifikáció kért, de nincs elérhető transcriber.", gpu)
            return
        ...
        if resolved_model_cls is None:
            logger.error(
                "Worker %s: Nem támogatott architektúra: %s.",
                gpu,
        ...
        narrator_audio, sample_rate = sf.read(args.narrator_ref_audio_path)
        if narrator_audio.ndim > 1:
            narrator_audio = narrator_audio.mean(axis=1)
        narrator_audio = narrator_audio.astype(np.float32, copy=False)

        eq_processed_ref_chunk = apply_eq_curve_to_audio(
            narrator_audio,
            sample_rate,
            args.eq_curve_settings,
        )
        base_ref_chunk_template = (
            normalize_peak(eq_processed_ref_chunk.copy(), args.ref_audio_peak)
            if args.normalize_ref_audio
            else eq_processed_ref_chunk.copy()
        )
        base_ref_text_template = args.narrator_ref_text
        initial_ref_duration = len(base_ref_chunk_template) / sample_rate
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            if normalize_fn:
                try:
                    ...
                    logger.error(
                        "Worker %s: Sikertelen normalizálás '%s' szegmensnél: %s",
                        gpu,
                        filename,
                        exc,
                        exc_info=True,
            ...
            base_ref_chunk = base_ref_chunk_template
            base_ref_text = base_ref_text_template
            original_ref_chunk = eq_processed_ref_chunk
            initial_duration = initial_ref_duration
...
def process_file_pair(args, input_wav_path: Path, input_json_path: Path) -> None:
    ...
    with mp.Manager() as manager:
        ...
        spawn_args = (args, chunks, stats, failed_segments_info)
...
def main() -> None:
    ...
    narrator_dir = Path(args.narrator)
    if not narrator_dir.is_absolute():
        narrator_dir = (PROJECT_ROOT / narrator_dir).resolve()
    else:
        narrator_dir = narrator_dir.resolve()
    try:
        narrator_audio, narrator_sample_rate, narrator_text, narrator_pair_count = load_narrator_reference_corpus(narrator_dir)
    except (FileNotFoundError, ValueError, RuntimeError) as exc:
        logger.error("Narrátor referencia betöltése sikertelen: %s", exc)
        sys.exit(1)

    narrator_tmp = tempfile.NamedTemporaryFile(prefix="f5_narrator_", suffix=".wav", delete=False)
    narrator_tmp_path = narrator_tmp.name
    narrator_tmp.close()
    sf.write(narrator_tmp_path, narrator_audio, narrator_sample_rate)
    args.narrator_ref_audio_path = narrator_tmp_path
    args.narrator_ref_text = narrator_text
    args.narrator_reference_dir = str(narrator_dir)
    logger.info(
        "Narrátor referencia betöltve: %s | összhossz: %.2f mp | minta: %d Hz | párok száma: %d",
        narrator_dir,
        len(narrator_audio) / narrator_sample_rate,
    ...
    try:
        ...
        temp_path = getattr(args, "narrator_ref_audio_path", None)
        if temp_path and os.path.exists(temp_path):
            try:
                os.remove(temp_path)
            except OSError:
                logger.warning("Nem sikerült törölni az ideiglenes narrátor fájlt: %s", temp_path)
