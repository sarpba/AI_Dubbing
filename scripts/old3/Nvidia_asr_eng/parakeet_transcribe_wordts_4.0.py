# ==============================================================================
#      NVIDIA PARAKEET‑TDT TRANSCRIPTION SCRIPT  (V3.9-SPLIT-PRO)
# ==============================================================================
#  🔊  Hosszú hangfájl‑transzkripció szó‑alapú időbélyegekkel (NVIDIA NeMo)
# ------------------------------------------------------------------------------
# Mit javít a v3.9-SPLIT-PRO?
#   • ✨ **Maximális mondathossz**: A `--max-segment-duration` paraméterrel
#     beállítható a mondatok maximális hossza (alapért.: 11.5s). A túl hosszú
#     mondatokat a szkript intelligensen, a legnagyobb szünetnél vágja el.
#   • ✨ Időbélyeg-finomítás (Padding) és Szünet-alapú mondattörés.
#   • 🐞 Minden korábbi bugfix és funkció (GPU-monitor, auto-chunk) változatlan.
# ==============================================================================
...
def process_directory_final_v3(directory_path: str, auto_chunk: bool, fixed_chunk_s: int, max_pause_s: float, padding_s: float, max_segment_s: float):
    ...
    for ap in wavs:
        ...
        res = transcribe_long_audio_final(
            asr_model, ap, chunk_len_s=chunk_len_s, batch_size=8, device=device,
...
def main():
    p = argparse.ArgumentParser(description="Transcribe audio files with NVIDIA NeMo (word‑level timestamps, v3.9-split-pro)")
    ...
    p.add_argument("--max-segment-duration", type=float, default=DEFAULT_MAX_SEGMENT_S, help=f"Mondatszegmensek max. hossza mp-ben (alapért.: {DEFAULT_MAX_SEGMENT_S}s). 0 a kikapcsoláshoz.")
    args = p.parse_args()
