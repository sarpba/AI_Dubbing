...
def split_text_with_ai(client, model, target_text, source_segments):
    ...
    if not target_text.strip(): return [seg['text'] for seg in source_segments]
    numbered_source_texts = "\n".join([f"{i+1}. {text}" for i, text in enumerate(source_texts_list)])
    system_prompt = "You are an expert in audio-visual localization... Your task is to intelligently split a single, complete Hungarian text block into multiple parts..."
    user_prompt = (f"Split the following Hungarian text into exactly {len(source_texts_list)} parts... --- YOUR REQUIRED OUTPUT (numbered list of split Hungarian parts) ---")
    ...
    try:
        ...
        if len(split_lines) == len(source_texts_list):
            ...
            logging.error(f"AI HIBA: Sorszámeltérés! Várt: {len(source_texts_list)}, Kapott: {len(split_lines)}."); return None
    except Exception as e:
...
def main(args):
    ...
    logging.info(f"--- TARTALOM-IGAZÍTÁS (v9 - Manuális korrekciós munkafolyamat) ---")
    # ... (többi info logolása)
    ...
    try:
        ...
        if not source_segments: logging.critical("A forrás JSON üres."); return
    except Exception as e:
        logging.critical(f"Hiba a forrás JSON olvasásakor: {e}"); return
    ...
    if narrator_srt_data:
        ...
        with open(narrator_filepath, 'w', encoding='utf-8') as f: json.dump(narrator_json, f, ensure_ascii=False, indent=2)
        logging.info(f"Narrátor fájl létrehozva: '{narrator_filepath}'")

    final_segments = source_data['segments']
    for i, seg in enumerate(final_segments): seg['original_index'] = i
    json_assigned = [False] * len(source_segments)
    srt_assigned = [False] * len(dialogue_srt_data)
    
    # Kezdetben mindenhol az eredeti szöveget hagyjuk
    for seg in final_segments:
        seg['translated_text'] = seg['text']

    # --- KORREKCIÓS FÁJL VAGY AUTOMATA FÁZISOK FUTTATÁSA ---
    if args.correction_file:
        logging.info(f"\n--- MANUÁLIS KORREKCIÓS FÁJL FELDOLGOZÁSA ---")
        try:
            with open(args.correction_file, 'r', encoding='utf-8') as f:
                corrections = json.load(f)
            
            for i, correction in enumerate(corrections):
                json_indices = correction['json_ids']
                srt_indices = correction['srt_ids']
                
                if any(json_assigned[j] for j in json_indices) or any(srt_assigned[s] for s in srt_indices):
                    logging.warning(f"A(z) {i+1}. korrekciós blokk már párosított elemet tartalmaz. Kihagyva.")
                    continue

                json_block = [source_segments[j] for j in json_indices]
                srt_block_text = " ".join([clean_srt_content(dialogue_srt_data[s].content) for s in sorted(srt_indices)])
                
                logging.info(f"Manuális blokk feldolgozása: JSON #{json_indices} -> SRT #{srt_indices}")
                
                split_texts = split_text_with_ai(client, args.model, srt_block_text, json_block)
                if split_texts:
                    for json_idx, text in zip(json_indices, split_texts):
                        final_segments[json_idx]['translated_text'] = text
                        json_assigned[json_idx] = True
                    for srt_idx in srt_indices:
                        srt_assigned[srt_idx] = True
        except Exception as e:
            logging.error(f"Hiba a korrekciós fájl feldolgozása közben: {e}")

    # 1. FÁZIS: NAGY PONTOSSÁGÚ 1:1 PÁROSÍTÁS
    logging.info("\n--- 1. FÁZIS: Nagy pontosságú 1:1 párosítás (középpont távolság alapján) ---")
    all_pairs = []
    for i, seg in enumerate(source_segments):
        if json_assigned[i]: continue
        seg_mid = seg['start'] + (seg['end'] - seg['start']) / 2
        for j, srt_entry in enumerate(dialogue_srt_data):
            if srt_assigned[j]: continue
    ...
    # Ha ez az első futtatás, itt megállunk és generáljuk a javítófájlt
    if not args.correction_file:
        base_filename = os.path.basename(source_json_path)
        output_filename = os.path.splitext(base_filename)[0] + '_aligned_partial.json'
        output_filepath = os.path.join(output_dir, output_filename)
        source_data['segments'] = final_segments
        with open(output_filepath, 'w', encoding='utf-8') as f:
            json.dump(source_data, f, ensure_ascii=False, indent=2)
        logging.info(f"\nAz elő-illesztett fájl elmentve: '{output_filepath}'")
    else: # Ha ez a második futtatás, folytatjuk a többi fázissal
        # 2. FÁZIS: TÖBB JSON -> 1 SRT (AI DARABOLÁS)
        logging.info("\n--- 2. FÁZIS: Több-az-egyhez (N:1) blokkok keresése ---")
        count_pass2 = 0
        # ... (A 2. fázis logikája itt következik, de csak a még nem párosított elemeken)
        ...
        count_pass3 = 0
        # ... (A 3. fázis logikája itt következik, de csak a még nem párosított elemeken)
        logging.info(f"3. Fázis befejezve. {count_pass3} db 1:N blokk összefűzve.")

        base_filename = os.path.basename(source_json_path)
        output_filename = os.path.splitext(base_filename)[0] + '_aligned_final.json'
        output_filepath = os.path.join(output_dir, output_filename)
        source_data['segments'] = final_segments
        with open(output_filepath, 'w', encoding='utf-8') as f:
            json.dump(source_data, f, ensure_ascii=False, indent=2)
        logging.info(f"\n--- VÉGLEGES FÁJL LÉTREHOZVA ---")
        logging.info(f"Fájl elmentve: '{output_filepath}'")

    # --- PÁROSÍTATLAN ELEMEK LISTÁZÁSA ---
    unassigned_json_indices = [i for i, assigned in enumerate(json_assigned) if not assigned]
    unassigned_srt_indices = [i for i, assigned in enumerate(srt_assigned) if not assigned]
    
    if unassigned_json_indices or unassigned_srt_indices:
        if not args.correction_file:
            correction_data = {
                "unassigned_json": [{"id": i, "start": s['start'], "end": s['end'], "text": s['text']} for i, s in enumerate(source_segments) if i in unassigned_json_indices],
                "unassigned_srt": [{"id": i, "start": s.start.total_seconds(), "end": s.end.total_seconds(), "text": clean_srt_content(s.content)} for i, s in enumerate(dialogue_srt_data) if i in unassigned_srt_indices]
            }
            correction_filepath = os.path.join(output_dir, 'correction_needed.json')
            with open(correction_filepath, 'w', encoding='utf-8') as f:
                json.dump(correction_data, f, ensure_ascii=False, indent=2)
            logging.warning(f"\n{len(unassigned_json_indices)} JSON és {len(unassigned_srt_indices)} SRT sor maradt párosítatlanul.")
            logging.warning(f"Manuális korrekcióhoz a '{correction_filepath}' fájl lett létrehozva.")
        
        logging.info("\n--- PÁROSÍTATLAN ELEMEK RÉSZLETES LISTÁJA ---")
        unassigned_json_details = [f"  JSON #{i} ({source_segments[i]['start']:.2f}-{source_segments[i]['end']:.2f}): '{source_segments[i]['text']}'" for i in unassigned_json_indices]
        unassigned_srt_details = [f"  SRT #{dialogue_srt_data[i].index} ({dialogue_srt_data[i].start.total_seconds():.2f}-{dialogue_srt_data[i].end.total_seconds():.2f}): '{clean_srt_content(dialogue_srt_data[i].content)}'" for i in unassigned_srt_indices]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Forrás JSON időzítésének intelligens, 3-fázisú illesztése egy cél SRT tartalmával, manuális korrekciós lehetőséggel.',
    ...
    parser.add_argument('-correction_file', required=False, help='Opcionális: A manuálisan létrehozott korrekciós JSON fájl elérési útja a második futtatáshoz.')
