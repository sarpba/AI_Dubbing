...
def get_dynamic_srt_context(batch_segments, srt_data, leeway_seconds=15.0, max_chars=1500):
    ...
    if relevant_subs:
        ...
        print(f"  -> Dinamikus SRT kontextus kikeresve a(z) {time.strftime('%H:%M:%S', time.gmtime(context_start_time))} - {time.strftime('%H:%M:%S', time.gmtime(context_end_time))} időablakhoz ({len(relevant_subs)} sor).")
        return sample
    return ""
...
def parse_numbered_lines(raw_text, expected_count):
    ...
    if len(numbered) != expected_count: return None
    return [re.sub(r'^\s*\d+[\.|\)]\s*', '', l).strip() for l in numbered]

def translate_batch(client, batch_segments, args, srt_data, batch_id_str):
    ...
    system_prompt = build_system_prompt(args.input_language, args.output_language, args.allow_sensitive_content, dynamic_srt_context, args.context)

    if args.save_prompt_to_file:
        try:
            full_prompt_content = (
                f"======================================================================\n"
                f"SYSTEM PROMPT (Instructions for the AI)\n"
                f"======================================================================\n\n"
                f"{system_prompt}\n\n"
                f"======================================================================\n"
                f"USER PROMPT (Content to translate for batch: {batch_id_str})\n"
                f"======================================================================\n\n"
                f"{text_block}\n"
            )
            with open(args.save_prompt_to_file, 'w', encoding='utf-8') as f:
                f.write(full_prompt_content)
            print(f"\n✅ Az első fordítási kérés sikeresen elmentve a(z) '{args.save_prompt_to_file}' fájlba.")
            print("A szkript most leáll a kérésnek megfelelően. API hívás nem történt.")
            sys.exit(0)
        except IOError as e:
    ...
    def _call():
        return client.chat.completions.create(model=args.model, messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": text_block}], temperature=0.1)
    try:
        ...
        if parsed is None:
            tightened_prompt = "Return ONLY a numbered list with EXACTLY the same number of lines as the input. No extra lines, no commentary, no code fences. Format: `1. ...`"
            def _call_tight():
                return client.chat.completions.create(model=args.model, messages=[{"role": "system", "content": tightened_prompt}, {"role": "user", "content": text_block}], temperature=0.0)
        ...
        if parsed is None:
            ...
            return None
        if args.stream:
            for line in parsed: print(f"    + {line}")
        return parsed
    except OpenAIError as e:
...
def pre_translate_from_srt(all_segments, srt_data, threshold):
    ...
    for i, segment in enumerate(all_segments):
        ...
        if best_match and smallest_diff <= threshold:
            ...
            pretranslated_progress[str(i)] = {"translated_text": srt_text_match, "text_hash": text_hash(text), "source": "pre-translated-from-srt"}
            available_srt.remove(best_match)
        else:
            remaining_segments.append(segment)
    match_count = len(pretranslated_progress)
    if match_count > 0: print(f"  -> Sikeresen párosítva és elő-fordítva {match_count} szegmens az SRT fájlból.")
...
def main(args):
    ...
    if args.use_openai:
        ...
        if 'gpt-' not in args.model.lower() and 'dall-e' not in args.model.lower():
             print(f"FIGYELMEZTETÉS: Az OpenAI API-t használja, de a modell neve ('{args.model}') nem tűnik OpenAI modellnek.")
        client = OpenAI(api_key=auth_key)
        print("OpenAI API kliens sikeresen beállítva.")
    else:
        print("Alapértelmezett módban a szkript a helyi LM Studiohoz csatlakozik (http://127.0.0.1:1234).")
    ...
    print(f"\nStíluskontextus keresése ('{args.output_language.upper()}') alapján a(z) '{upload_dir}' mappában...")
    ...
    if not untranslated_segments:
        ...
        for i, batch in enumerate(batches):
            ...
            translated_batch_lines = translate_batch(client, batch, args, srt_data, f"{i+1}")
            if translated_batch_lines is None:
                ...
                first_res = translate_batch(client, first_half, args, srt_data, f"{i+1}-A")
                second_res = translate_batch(client, second_half, args, srt_data, f"{i+1}-B")
            ...
            for segment_obj, translated_line in zip(batch, translated_batch_lines):
                progress[str(segment_obj['original_index'])] = {"translated_text": translated_line, "text_hash": segment_obj.get('_text_hash') or text_hash(segment_obj.get('text', ''))}
    ...
    for idx, segment in enumerate(data['segments']):
        ...
        if isinstance(entry, dict):
            ...
            if entry.get('source') != 'pre-translated-from-srt':
                old_hash, new_hash = entry.get('text_hash'), text_hash(segment.get('text', ''))
                if old_hash and new_hash and old_hash != new_hash:
                    print(f"Figyelmeztetés: A(z) {idx}. szegmens forrásszövege megváltozott a fordítás óta.")
    ...
    if os.path.exists(progress_filepath):
        try:
            ...
            print(f"Figyelmeztetés: A haladási fájl ({progress_filepath}) törlése sikertelen: {e}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Szövegszegmensek intelligens fordítása dinamikus SRT kontextussal.', formatter_class=argparse.RawTextHelpFormatter)
    ...
    api_group.add_argument('-model', default='unsloth/gpt-oss-20b', required=False, help='A használni kívánt modell neve. Alapértelmezett: unsloth/gpt-oss-20b')
    ...
    pre_trans_group.add_argument('--pre-translate-threshold', type=float, default=0.3, help='Időbélyeg-eltérés küszöbértéke az elő-fordításhoz. Alapértelmezett: 0.3s')
    other_group = parser.add_argument_group('Other and Debugging Options')
    other_group.add_argument('-stream', action='store_true', help='Bekapcsolja a soronkénti kimenetet a konzolon.')
    other_group.add_argument('--save-prompt-to-file', type=str, metavar='FILENAME', help='Elmenti az első AI kérést a megadott fájlba, majd kilép (nincs API hívás).')
    try:
        other_group.add_argument('--allow-sensitive-content', action=argparse.BooleanOptionalAction, default=True, help='Hű fordítás kényes tartalmaknál is. Kikapcsolás: --no-allow-sensitive-content')
    except Exception:
