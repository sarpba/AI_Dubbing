...
def should_emit_live_status() -> bool:
...
def should_emit_snapshot(
    ...
    processed = int(stats_snapshot.get("successful", 0)) + int(stats_snapshot.get("failed", 0))
    if processed <= last_processed:
        return False
    step = max(10, total_segments // 20) if total_segments > 0 else 10
    return processed == 1 or processed - last_processed >= step or processed >= total_segments
...
def run_segments_on_device(
    ...
    def process_single_segment(segment: Dict[str, object]) -> None:
        ...
        current_snapshot = snapshot_stats(stats, lock=stats_lock)
        if emit_status:
            emit_status_line(current_snapshot)
        elif should_emit_snapshot(current_snapshot, last_snapshot_processed, int(current_snapshot.get("total", 0))):
            print(render_status_line(current_snapshot), flush=True)
            last_snapshot_processed = int(current_snapshot.get("successful", 0)) + int(
                current_snapshot.get("failed", 0)
            )
    ...
    if resolved_device == "cuda" and torch.cuda.is_available():
        ...
        if num_workers > 1:
            ...
            with mp.Manager() as manager:
                ...
                live_status_enabled = should_emit_live_status()
                last_snapshot_processed = 0
                last_status_line = ""
                while not process_context.join(timeout=0.5):
                    ...
                    if live_status_enabled:
                        ...
                        continue
                    if should_emit_snapshot(current_snapshot, last_snapshot_processed, total_segments):
                ...
                if live_status_enabled and final_line != last_status_line:
                    emit_status_line(final_snapshot, final=True)
                elif live_status_enabled:
                    print("", flush=True)
                elif not should_emit_snapshot(final_snapshot, last_snapshot_processed, total_segments):
            ...
            run_segments_on_device(
                ...
                emit_status=(not args.debug and should_emit_live_status()),
            )
