...
def save_api_key(project_root: Path, api_key: str) -> None:
    """Persist the OpenRouter API key to keyholder.json using base64 encoding."""
...
def load_api_key(project_root: Path) -> Optional[str]:
    ...
    try:
        with open(keyholder_path, "r", encoding="utf-8") as fp:
            data = json.load(fp)
        encoded_key = data.get(OPENROUTER_KEY_FIELD)
        if not encoded_key:
            return None
...
def load_glossary(glossary_path: Optional[str], project_root: Path) -> Optional[Dict[str, str]]:
    """Load glossary mappings from a JSON file."""
...
def build_system_prompt(
    ...
    """Construct a strategy-aware system prompt (override keeps formatting support)."""
    ...
    if override and override.strip():
        ...
        try:
            return system_prompt.format(**replacements)
        except KeyError as exc:
    ...
    if locale == "HU":
        ...
        instructions.append(
            "Fordíts minden számozott sort külön-külön, tartsd meg a számozást és a sorrendet (formátum: `1. fordítás`). Ne vond össze vagy tördelj máshol."
        )
        instructions.append(
    ...
    if allow_sensitive:
        instructions.append(
            (
                "Ne cenzúrázz és ne hagyd ki a durva kifejezéseket, csak akkor enyhíts, ha az eredeti is enyhe."
                if locale == "HU"
                else "Do not censor or soften profanity unless the source already does; match the original intensity."
            )
        )

    if tone:
        instructions.append(("Elvárt hangnem/stílus: " if locale == "HU" else "Requested tone/register: ") + tone.strip())
    if target_audience:
        instructions.append(("Célközönség: " if locale == "HU" else "Target audience: ") + target_audience.strip())
    if platform:
        platform_note = platform.strip()
        if locale == "HU":
            instructions.append(
                "Platform/specifikus szempontok: "
                f"{platform_note}. Igazítsd a szóhasználatot és sorhosszakat az adott felület tipikus elvárásaihoz."
            )
        else:
            instructions.append(
    ...
    if context:
        instructions.append(("Kontekstus: " if locale == "HU" else "Context: ") + context.strip())
    if style_notes:
        instructions.append(
...
def build_openrouter_client(api_key: str) -> OpenAI:
    """Instantiate the OpenRouter-compatible OpenAI client."""
...
def main(
    ...
    if not auth_key:
        logger.error("Hiba: Nincs elérhető OpenRouter API kulcs. Add meg a --auth-key paramétert.")
        sys.exit(1)
    ...
    if style_notes:
        logger.info("  - Stílus jegyzet:        %s", style_notes)
    ...
    metadata["translation_provider"] = "openrouter"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Projekt-alapú JSON fordítás OpenRouter kompatibilis modellekkel. "
            "A config.json alapján keresi a feldolgozandó fájlokat."
        )
    )
    parser.add_argument(
        "-p",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "A bemeneti nyelv kódja (pl. EN, HU). Ha nincs megadva, a config.json default_source_lang értéke lesz használva."
        ),
    )
    parser.add_argument(
        "-output_language",
        ...
        default=None,
        help=(
            "A kimeneti nyelv kódja (pl. EN, HU). Ha nincs megadva, a config.json default_target_lang értéke lesz használva."
        ),
    )
    parser.add_argument(
        "-context",
    ...
    parser.add_argument(
        ...
        help="A használni kívánt OpenRouter modell neve. Alapértelmezett: google/gemini-2.0-flash-001.",
    )
    parser.add_argument(
        "-stream",
        "--stream",
    ...
    parser.add_argument(
        ...
        dest="auth_key",
        default=None,
        help="Az OpenRouter API kulcsa. Megadás esetén elmentődik a keyholder.json fájlba.",
    )
    parser.add_argument(
        "-systemprompt",
        "--systemprompt",
