...
This script provides advanced tuning options for potentially higher accuracy.
...
-------
```bash
python diarize2.py -i movie.wav        # auto (CUDA if possible)
python diarize2.py -i movie.wav --device cuda

# Example with advanced tuning:
python diarize2.py -i movie.wav --min-speakers 2 --max-speakers 3 \
       --segmentation-threshold 0.4 --clustering-threshold 0.6 \
       --segmentation-min-duration-off 0.2 --segmentation-min-duration-on 0.1
```

Workflow
--------
1. Convert the input audio (`-i`) to 16 kHz / mono WAV (if needed).
2. Run pyannote speaker-diarization on the selected device with specified hyperparameters.
...
def diarize(
    ...
    segmentation_min_duration_on: float | None = None,
    ...
    if segmentation_min_duration_on is not None:
        seg_overrides["min_duration_on"] = segmentation_min_duration_on
    if seg_overrides:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Add speaker labels to Whisper JSON with advanced tuning")
    ...
    parser.add_argument(
        "--segmentation-min-duration-on", type=float, default=None,
        help="Minimum duration of speech turn (seconds) for segmentation. Default: model's own."
    )
    ...
    tracks = diarize(
        ...
        segmentation_min_duration_on=args.segmentation_min_duration_on,
    )
    ...
    # Also add speaker to top-level word_segments if present
    if "word_segments" in data and isinstance(data["word_segments"], list):
        console.print("[cyan]🎤 Adding speaker labels to top-level 'word_segments' …")
        for word_seg in data["word_segments"]:
            if "start" in word_seg:
                try:
                    word_start_time = float(word_seg["start"])
                    word_seg["speaker"] = speaker_for_time(tracks, word_start_time)
                except ValueError:
                    console.print(f"[yellow]⚠ Skipping word_segment due to invalid start time: {word_seg}")
            else:
                console.print(f"[yellow]⚠ Skipping word_segment due to missing 'start' key: {word_seg}")
