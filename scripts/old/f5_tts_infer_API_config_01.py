...
class F5TTS:
    def __init__(
        ...
        use_ema: bool = True, # Ideiglenesen hagyjuk alapértelmezett értéken, ha nincs a configban
        device: str = None,
    ):
        ...
        # EMA beolvasása a configból, ha van, különben az alapértelmezett értéket használja
        # Ezt az 'use_ema' paramétert most már a konfigurációból is be tudjuk olvasni, ha létezik az 'ema' kulcs
        self.use_ema = model_config["model"].get("ema", {}).get("enabled", use_ema)
        ...
        self.load_ema_model(
            ...
            vocab_file,
            ode_method,
            self.use_ema # Itt használjuk a configból vagy alapértelmezettből származó értéket
        )
    ...
    def load_ema_model(self, model_section_cfg: dict, ckpt_file: str, vocab_file: str, ode_method: str, use_ema: bool):
        ...
        # --- JAVÍTOTT RÉSZ: Szűrjük ki a nem kívánt/fel nem ismert paramétereket ---
        filtered_model_arch_cfg = model_arch_cfg.copy()

        # Azok a kulcsok, amelyekről tudjuk, hogy problémát okozhatnak a régebbi DiT verziókban
        # Az "attn_backend" okozta a konkrét hibát. A "qk_norm" is egy újabb paraméter.
        # Ha később más paraméterek is hibát okoznak (pl. TypeError: unexpected keyword argument 'X'),
        # akkor azokat is ide kell adni.
        keys_to_remove = [
            "attn_backend",
            "qk_norm",
            # "text_mask_padding", # Ezeket NEM feltétlenül kell eltávolítani,
            # "pe_attn_head"       # ha a régi DiT osztály is ismerte, csak az értékük más.
                                 # Az hiba "unexpected keyword argument", ami a kulcsra vonatkozik.
        ]

        for key in keys_to_remove:
            if key in filtered_model_arch_cfg:
                logger.warning(f"Removing '{key}' from DiT model config as it might not be supported by the current DiT version in f5_tts library.")
                del filtered_model_arch_cfg[key]
        # --- JAVÍTOTT RÉSZ VÉGE ---

        logger.info(f"Loading EMA model: {model_backbone} with arch config: {filtered_model_arch_cfg}") # Logoljuk a szűrt configot
        self.ema_model = load_model(
            model_cls,
            filtered_model_arch_cfg, # ITT HASZNÁLJUK A SZŰRT KONFIGOT
            ckpt_file,
            self.mel_spec_type,
            vocab_file,
            ode_method,
...
def parse_arguments():
    ...
    parser.add_argument("--config_file", type=str, required=True, help="Path to the model configuration JSON file (e.g., /path/to/config.json)")
...
def main_worker(gpu, args, all_chunks, input_wav_path_str, model_config):
...
def process_file_pair(args, input_wav_path, input_json_path, model_config):
    ...
    spawn_args = (args, chunks, str(input_wav_path), model_config)
...
def main():
    ...
    process_file_pair(args, input_wav_path, input_json_path, model_config)
