...
def main_worker(gpu, args, all_chunks, input_wav_path_str, stats, failed_segments_info):
    ...
    try:
        ...
        if args.seed == -1:
            ...
            # --- JAVÍTOTT, EXPLICIT FÜGGVÉNYHÍVÁSOK ---
            if is_huggingface_model:
                if pipeline is None:
                    logger.error("Hugging Face model requested, but `transformers` is not installed."); return
                ...
                if whisper is None:
                    logger.error("OpenAI model requested, but `openai-whisper` is not installed."); return
                    ...
                    try:
                        ...
                        if dist <= final_tolerance:
                            ...
                            if args.double_ref_on_failure and attempt == 1:
                                current_ref_text=f"{original_ref_text} {original_ref_text}"; current_ref_audio_chunk=np.concatenate([current_ref_audio_chunk, current_ref_audio_chunk])
