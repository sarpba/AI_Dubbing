...
def process_json_file(args):
    ...
    try:
        ...
        # Adjust timestamps based on user rules before processing segments
        if "segments" in json_content and isinstance(json_content["segments"], list) and len(json_content["segments"]) > 0:
            # Rule 1: Adjust start time of the first segment
            try:
                original_start_str = json_content["segments"][0].get('start')
                if original_start_str is not None:
                    original_start = float(original_start_str)
                    adjusted_start = max(0.0, original_start - 0.2)
                    json_content["segments"][0]['start'] = adjusted_start
                    if abs(original_start - adjusted_start) > 1e-6 : # Log only if changed significantly
                        logging.info(f"Adjusted start for first segment in '{base_name}': {original_start:.3f}s -> {adjusted_start:.3f}s")
            except (ValueError, TypeError) as e:
                logging.warning(f"Could not parse start time for first segment in '{base_name}': {original_start_str}. Error: {e}")
            except KeyError as e:
                logging.warning(f"Missing 'start' key for first segment in '{base_name}'. Error: {e}")


            # Rules 2 and 3: Adjust gaps between segments
            for i in range(len(json_content["segments"]) - 1):
                current_segment_dict = json_content["segments"][i]
                next_segment_dict = json_content["segments"][i+1]

                try:
                    prev_seg_end_time_str = current_segment_dict.get('end')
                    next_seg_start_time_str = next_segment_dict.get('start')
                    current_seg_start_time_str = current_segment_dict.get('start') # For boundary check
                    next_seg_end_time_str = next_segment_dict.get('end') # For boundary check

                    if None in [prev_seg_end_time_str, next_seg_start_time_str, current_seg_start_time_str, next_seg_end_time_str]:
                        logging.warning(f"Missing time key(s) for segments {i}, {i+1} in '{base_name}'. Skipping adjustment for this pair.")
                        continue

                    prev_seg_end_time = float(prev_seg_end_time_str)
                    next_seg_start_time = float(next_seg_start_time_str)
                    current_seg_start_time = float(current_seg_start_time_str)
                    next_seg_end_time = float(next_seg_end_time_str)

                    gap = next_seg_start_time - prev_seg_end_time

                    if gap < 0: # Overlap
                        logging.warning(f"Overlap detected between segment {i} (end: {prev_seg_end_time:.3f}) and segment {i+1} (start: {next_seg_start_time:.3f}) in '{base_name}'. Gap: {gap:.3f}s. No adjustment made.")
                        continue

                    original_prev_end = prev_seg_end_time
                    original_next_start = next_seg_start_time

                    if gap > 0.5: # Rule 2
                        adjusted_prev_end = prev_seg_end_time + 0.2
                        adjusted_next_start = next_seg_start_time - 0.2
                        
                        if adjusted_next_start > adjusted_prev_end and \
                           adjusted_prev_end > current_seg_start_time and \
                           adjusted_next_start < next_seg_end_time:
                            current_segment_dict['end'] = adjusted_prev_end
                            next_segment_dict['start'] = adjusted_next_start
                            logging.info(f"Rule 2: Adjusted gap (>0.5s) for segments {i}({original_prev_end:.3f}) and {i+1}({original_next_start:.3f}) in '{base_name}'. New: {adjusted_prev_end:.3f} - {adjusted_next_start:.3f}. Original gap: {gap:.3f}s")
                        else:
                            logging.warning(f"Rule 2 adjustment for {i}-{i+1} in '{base_name}' (gap {gap:.3f}s) would create invalid times ({adjusted_prev_end:.3f} vs {adjusted_next_start:.3f}) or cross segment boundaries. Attempting Rule 3 logic.")
                            # Fallback to Rule 3 logic if Rule 2 fails (gap is still > 0.05)
                            if gap > 0.05: # Check again, as the original gap was > 0.5
                                new_prev_end = (original_prev_end + original_next_start - 0.05) / 2
                                new_next_start = new_prev_end + 0.05
                                if new_next_start > new_prev_end and \
                                   new_prev_end >= original_prev_end - 1e-9 and new_prev_end < original_next_start + 1e-9 and \
                                   new_next_start > original_prev_end - 1e-9 and new_next_start <= original_next_start + 1e-9 and \
                                   new_prev_end > current_seg_start_time -1e-9 and new_next_start < next_seg_end_time + 1e-9:
                                    current_segment_dict['end'] = new_prev_end
                                    next_segment_dict['start'] = new_next_start
                                    logging.info(f"Rule 2 Fallback (Rule 3 logic): Adjusted gap for segments {i}({original_prev_end:.3f}) and {i+1}({original_next_start:.3f}) in '{base_name}'. New: {new_prev_end:.3f} - {new_next_start:.3f}. Original gap: {gap:.3f}s")
                                else:
                                    logging.warning(f"Rule 2 Fallback (Rule 3 logic) failed for segments {i}-{i+1} in '{base_name}'. Original times kept. Original gap: {gap:.3f}s. Conditions: new_prev_end ({new_prev_end:.3f}) vs current_seg_start_time ({current_seg_start_time:.3f}), new_next_start ({new_next_start:.3f}) vs next_seg_end_time ({next_seg_end_time:.3f})")
                            else: # Gap was > 0.5 but Rule 2 failed, and now gap is not > 0.05 for Rule 3 fallback
                                logging.info(f"Rule 2 failed for segments {i}-{i+1} in '{base_name}', and gap {gap:.3f}s not eligible for Rule 3 fallback. Original times kept.")
                    
                    elif gap > 0.05: # Rule 3 (0.05s < gap <= 0.5s)
                        new_prev_end = (original_prev_end + original_next_start - 0.05) / 2
                        new_next_start = new_prev_end + 0.05
                        ...
                        if new_next_start > new_prev_end and \
                           ...
                           new_prev_end > current_seg_start_time - 1e-9 and new_next_start < next_seg_end_time + 1e-9:
                            current_segment_dict['end'] = new_prev_end
                            next_segment_dict['start'] = new_next_start
                            logging.info(f"Rule 3: Adjusted gap (0.05 < gap <= 0.5s) for segments {i}({original_prev_end:.3f}) and {i+1}({original_next_start:.3f}) in '{base_name}'. New: {new_prev_end:.3f} - {new_next_start:.3f}. Original gap: {gap:.3f}s")
                        else:
                            logging.warning(f"Rule 3 adjustment for segments {i}-{i+1} in '{base_name}' (gap {gap:.3f}s) failed. Resulted in invalid times ({new_prev_end:.3f} vs {new_next_start:.3f}) or crossed boundaries. Original times kept. Conditions: new_prev_end ({new_prev_end:.3f}) vs current_seg_start_time ({current_seg_start_time:.3f}), new_next_start ({new_next_start:.3f}) vs next_seg_end_time ({next_seg_end_time:.3f})")
                    
                    elif gap >= 0: # Handles gap <= 0.05 but non-negative
                        logging.debug(f"Gap between segment {i} and {i+1} in '{base_name}' is {gap:.3f}s (<=0.05s). No adjustment needed.")

                except (ValueError, TypeError) as e:
                    logging.warning(f"Error parsing times for segments {i}, {i+1} in '{base_name}' during gap adjustment: {e}. Skipping this pair.")
                except KeyError as e: # Should be caught by None check above, but as a safeguard
                    logging.warning(f"Missing key {e} for segments {i}, {i+1} in '{base_name}' during gap adjustment. Skipping this pair.")
        # --- End of timestamp adjustments ---
