...
def run_dubbing_workflow(project_name, project_path, config, params, memory_handler):
    ...
    steps = [
        ...
        ([sync_python, 'scripts/split_segments_by_speaker2.py', '-p', project_name], "[split_by_speaker]", True),
    ...
    for cmd, prefix, is_critical in steps:
        ...
        if prefix == "[unpack_srt]":
            try:
                ...
                pass # Ha a könyvtár nem létezik, a lépést futtatni kell
...
def main():
    ...
    if args.input_dir:
        ...
        for i, video_path in enumerate(video_files):
            ...
            # Projekt struktúra létrehozása/ellenőrzése, mielőtt bármit csinálnánk
            ...
            # Ellenőrizzük, van-e már videó a projekt 'upload' mappájában
            ...
            try:
                # A run_dubbing_workflow-nak átadjuk a project_path-t is
        ...
        # Ahelyett, hogy hibával leállnánk, létrehozzuk/ellenőrizzük a struktúrát
        create_project_structure(project_path, config['PROJECT_SUBDIRS'])
        upload_dir = os.path.join(project_path, config['PROJECT_SUBDIRS']['upload'])
        
        memory_handler, file_handler = setup_logging(project_path, config['PROJECT_SUBDIRS']['logs'], project_name)
        try:
            # Ellenőrizzük, van-e videófájl
            has_video = any(f.lower().endswith(VIDEO_EXTENSIONS) for f in os.listdir(upload_dir))
