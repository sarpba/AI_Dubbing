...
def main():
    parser = argparse.ArgumentParser(
        description='Audiófájlok szétválasztása Demucs/MDX segítségével.'
    )
    ...
    for filename in os.listdir(input_dir):
        ...
        # Ha WAV fájl, akkor közvetlenül használjuk
        if filename.lower().endswith('.wav'):
            temp_audio_path = audio_path
            success = True
        else:
            # Egyéb formátum esetén konvertálunk WAV-be
            success = extract_audio(audio_path, temp_audio_path)
        if not success:
            continue
        ...
        # Fájlok áthelyezése és törlése
        # 1. Töröljük a full.wav fájlt ha nem kell megtartani
        if not args.keep_full_audio:
            full_path = os.path.join(output_dir, f"{base_name}_full.wav")
            if os.path.exists(full_path):
                os.remove(full_path)
        
        # 2. Áthelyezzük a _non_speech.wav fájlt a background mappába
        non_speech_files = [f for f in os.listdir(output_dir) if f.endswith('_non_speech.wav')]
        for non_speech_file in non_speech_files:
            non_speech_path = os.path.join(output_dir, non_speech_file)
            # A config.json-ből kiolvassuk a separated_audio_background mappa elérési útját
            with open('config.json') as f:
                config = json.load(f)
            background_dir = os.path.join(os.path.dirname(output_dir), config['PROJECT_SUBDIRS']['separated_audio_background'])
            os.makedirs(background_dir, exist_ok=True)
            shutil.move(non_speech_path, os.path.join(background_dir, os.path.basename(non_speech_path)))
            print(f"Áthelyezve: {non_speech_path} -> {background_dir}")
        
        # 3. Töröljük az ideiglenes fájlt ha nem a bemeneti WAV volt
        if temp_audio_path != audio_path and os.path.exists(temp_audio_path):
            os.remove(temp_audio_path)

    print("Feldolgozás kész. A szétválasztott fájlok:")
    print(f"- Beszéd: {os.path.join(output_dir, f'{base_name}_speech.wav')}")
    # A config.json-ből kiolvassuk a separated_audio_background mappa elérési útját
    with open('config.json') as f:
        config = json.load(f)
    background_dir = os.path.join(os.path.dirname(output_dir), config['PROJECT_SUBDIRS']['separated_audio_background'])
    print(f"- Háttérzaj: {os.path.join(background_dir, f'{base_name}_non_speech.wav')}")
