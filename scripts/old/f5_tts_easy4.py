...
import re
import math
import numpy as np  # <-- ÚJ IMPORT A HANGFÁJLOK ÖSSZEFŰZÉSÉHEZ
...
def time_to_filename_str(seconds: float) -> str:
    ...
    minutes, secs = divmod(td.seconds, 60); hours, minutes = divmod(minutes, 60); milliseconds = td.microseconds // 1000
...
def parse_arguments():
    ...
    # === ÚJ KAPCSOLÓ A REFERENCIA MEGKÉTSZEREZÉSÉHEZ ===
...
def main_worker(gpu, args, all_chunks, input_wav_path_str, stats):
    ...
    try:
        ...
        if args.seed == -1:
            # ... (Verifikációs eszközök betöltése változatlan) ...
            whisper_language=NORMALIZER_TO_WHISPER_LANG.get(args.norm.lower());is_huggingface_model='/' in args.whisper_model
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            original_ref_text = segment.get('text', '').strip() # Eredeti mentése
            gen_text = segment.get('translated_text', '').strip()
            
            if not all([isinstance(start_time,(int,float)),isinstance(end_time,(int,float)),original_ref_text,gen_text]): continue
            ...
            # Munkaváltozók inicializálása az eredeti adatokkal
            current_ref_text = original_ref_text
            current_ref_audio_chunk = full_audio_data[start_sample:end_sample]
            ...
            if args.seed == -1:
                ...
                for attempt in range(1, args.max_retries + 1):
                    ...
                    try:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".wav",delete=True) as tmp_ref:
                            # Mindig az aktuális (akár duplázott) referenciát használjuk
                        ...
                        if dist <= final_tolerance:
                            ...
                            # === REFERENCIA DUPLÁZÁSI LOGIKA ===
                            if args.double_ref_on_failure and attempt == 1:
                                logger.info(f"Worker {gpu}: First failure. Doubling reference for subsequent retries.")
                                current_ref_text = f"{original_ref_text} {original_ref_text}"
                                current_ref_audio_chunk = np.concatenate([current_ref_audio_chunk, current_ref_audio_chunk])

                            # === HIBÁS GENERÁLÁS MENTÉSE ===
                ...
                try:
                    ...
                    logger.error(f"Worker {gpu}: Direct generation FAILED: {e}", exc_info=False); stats['failed'] += 1
...
def process_file_pair(args, input_wav_path, input_json_path):
    ...
    with mp.Manager() as manager:
        ...
        mp.spawn(main_worker, nprocs=num_workers, args=(args, chunks, str(input_wav_path), stats))
