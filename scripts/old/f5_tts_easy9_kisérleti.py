...
def parse_arguments():
    ...
    parser.add_argument("--phonetic-ref", action="store_true", help="A referencia szöveget fonetikus átírással használja.")
    parser.add_argument("--normalize-ref-audio", action="store_true", help="Referencia audio csúcshangerejének normalizálása.")
    parser.add_argument("--ref-audio-peak", type=float, default=0.95, help="Normalizált referencia audio cél csúcsértéke (0.0-1.0).")
    parser.add_argument("--max_workers", type=int, default=None, help="Párhuzamos workerek maximális száma.")
    parser.add_argument("--seed", type=int, default=-1, help="Véletlenszám-generátor magja. '-1' = verifikációs mód.")
    parser.add_argument("--max-retries", type=int, default=5, help="Maximális újrapróbálkozások (csak random seed).")
    parser.add_argument("--tolerance-factor", type=float, default=1.0, help="Tolerancia szorzó szó szám alapján.")
    parser.add_argument("--min-tolerance", type=int, default=2, help="Tolerancia minimuma.")
    parser.add_argument("--whisper-model", type=str, default="openai/whisper-large-v3", help="ASR modell.")
    parser.add_argument("--beam-size", type=int, default=5, help="Whisper beam search szélesség.")
    parser.add_argument("--save-failures", action="store_true", help="Hibás generálások mentése.")
    parser.add_argument("--double-ref-on-failure", action="store_true", help="Első hiba után ref duplázás 0.4s csenddel, max 11.5s-ig.")
    parser.add_argument("--extend_shorts", action="store_true", help="2s alatti ref ismétlése 0.4s csöndekkel 2s fölé már az első generálás előtt.")
...
def extend_short_ref(ref_audio: np.ndarray, ref_text: str, sample_rate: int, min_seconds: float = 2.0) -> tuple[np.ndarray, str, bool]:
    """
    Ha a ref_audio rövidebb mint min_seconds, akkor ismétli (2..4x),
    és minden illesztés közé 0.4s csendet tesz.
    """
...
def normalize_for_levenshtein_custom(text: str) -> str:
    """
    Speciális normalizálás a Levenshteinhez:
    - általános normalize_text_for_comparison
    - 'ly' -> 'j' helyettesítés, hogy 'ly' és 'j' ne számítson eltérésnek
    """
    t = normalize_text_for_comparison(text)
    t = t.replace("ly", "j")
    return t
...
def main_worker(gpu, args, all_chunks, input_wav_path_str, stats, failed_segments_info):
    ...
    try:
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            # Rövid referencia kiterjesztése már első generálás előtt, 0.4s csöndekkel
            ...
            # A Levenshtein-hez használt normalizált gen_text (ly/j egyenlőség kezeléssel)
            norm_gen_text_for_log = normalize_text_for_comparison(gen_text)
            norm_gen_text_for_lev = normalize_for_levenshtein_custom(gen_text)

            if args.seed == -1:
                verification_passed = False

                best_valid = None  # {'attempt': i, 'dist': d, 'tmp_path': path, 'asr_text': str}
                tol = None
                ...
                for attempt in range(1, args.max_retries + 1):
                    ...
                    try:
                        ...
                        # ASR szöveg normalizálása és ly/j-egyenlőség
                        asr_text_norm = normalize_text_for_comparison(converted_transcribed_text)
                        asr_text_lev = normalize_for_levenshtein_custom(converted_transcribed_text)

                        dist = levenshtein_distance(norm_gen_text_for_lev, asr_text_lev)

                        original_words = norm_gen_text_for_lev.split()
                        transcribed_words = asr_text_lev.split()
                        if original_words and transcribed_words:
                            if transcribed_words[0] in ('a', 'e') and transcribed_words[0] != original_words[0]:
                                ...
                                if norm_gen_text_for_lev.startswith(rest_of_transcribed):
                                    dist += 5
                        ...
                        print_attempt_log_md(
                            ...
                            asr_text=asr_text_norm,
                            tol_allowed=tol,
                            dist=dist,
                            success=success
                        )

                        # Ha sikeres és a távolság 1 vagy kisebb:
                        # - ha 1: azonnal elfogadjuk és továbblépünk a következő splitre
                        # - ha 0: ugyanígy azonnal elfogadjuk
                        if success and dist <= 1:
                            shutil.move(temp_gen_path, str(output_wav_path))
                            verification_passed = True
                            break

                        # Ha sikeres, de >1 (tehát 2..tol), gyűjtsük a legjobbat,
                        # és FOLYTASSUK a generálást a felhasználói kérés szerint.
                        if success:
                            if (best_valid is None) or (dist < best_valid['dist']):
                                # töröljük az előző legjobb ideiglenes fájlt
                                if best_valid is not None and os.path.exists(best_valid['tmp_path']):
                                    try:
                                        os.remove(best_valid['tmp_path'])
                                    except Exception:
                                        pass
                                best_valid = {
                                    'attempt': attempt,
                                    'dist': dist,
                                    'tmp_path': temp_gen_path,
                                    'asr_text': asr_text_norm
                                }
                            else:
                                # ez is sikeres, de nem jobb a legjobbnál, törölhetjük
                                try:
                                    os.remove(temp_gen_path)
                                except Exception:
                                    pass
                        else:
                            # sikertelen eset mentése vagy törlése
                        ...
                        if temp_gen_path and os.path.exists(temp_gen_path):
                            try:
                                ...
                                pass
                        # Hiba esetén is írjunk egy log blokkot
                        print_attempt_log_md(
                            device_str=device,
                            attempt_idx=attempt,
                            max_attempts=args.max_retries,
                            ...
                            tol_allowed=(tol if tol is not None else 0),
                            dist=(dist if dist != -1 else 0),
                            success=False
                        )
                        continue

                # Ha NEM találtunk <=1-eset az összes próbálkozás alatt,
                # de voltak érvényesek (<= tol), akkor a LEGALACSONYABB distűt fogadjuk el.
                if not verification_passed and best_valid is not None:
                    shutil.move(best_valid['tmp_path'], str(output_wav_path))
                    verification_passed = True
                ...
                try:
                    ...
                    print_attempt_log_md(
                        ...
                        phon_ref_text=current_phon_text,
                        gen_text=gen_text,
                        norm_gen_text=normalize_text_for_comparison(gen_text),
