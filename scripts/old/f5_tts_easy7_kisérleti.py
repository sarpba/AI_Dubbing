...
def parse_arguments():
    ...
    # === ÚJ ARGUMENTUMOK A REFERENCIA HANGERŐ NORMALIZÁLÁSÁHOZ ===
    ...
    parser.add_argument("--ref-audio-peak", type=float, default=0.95, help="A normalizált referencia audio cél csúcsértéke (0.0-1.0). Csak a --normalize-ref-audio kapcsolóval együtt érvényes.")
    # Worker and verification arguments
    ...
    parser.add_argument("--double-ref-on-failure", action="store_true", help="Az első hibás generálási kísérlet után megduplázza a referencia audiót és szöveget a további próbálkozásokhoz.")
    args = parser.parse_args()
    return args

# === ÚJ SEGÉDFÜGGVÉNY A CSÚCS-NORMALIZÁLÁSHOZ ===
def normalize_peak(audio: np.ndarray, target_peak: float) -> np.ndarray:
    """Scales an audio signal so its peak amplitude matches the target peak."""
    if not 0.0 < target_peak <= 1.0:
        logger.warning(f"Target peak {target_peak} is outside the valid range (0.0, 1.0]. Clipping to 0.95.")
        target_peak = 0.95
    
    current_peak = np.max(np.abs(audio))
    if current_peak == 0:
        return audio # Csendes audio, nincs mit normalizálni
...
def main_worker(gpu, args, all_chunks, input_wav_path_str, stats, failed_segments_info):
    ...
    try:
        ...
        if args.seed == -1:
            if num2words is None: logger.error("Verification requires 'num2words'. Please 'pip install num2words'."); return
        ...
        # === REFERENCIA AUDIO NORMALIZÁLÁSA A FELDOLGOZÁS ELŐTT ===
        if args.normalize_ref_audio:
            logger.info(f"Worker {gpu}: Normalizing reference audio to a peak of {args.ref_audio_peak}...")
            full_audio_data = normalize_peak(full_audio_data, args.ref_audio_peak)
            logger.info(f"Worker {gpu}: Reference audio normalized.")
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            current_ref_text=original_ref_text; current_ref_audio_chunk=full_audio_data[start_sample:end_sample]

            if args.seed == -1:
                # ... (A verifikációs ciklus logikája változatlan) ...
                ...
                for attempt in range(1, args.max_retries+1):
                    ...
                    try:
                        ...
                        if dist <= final_tolerance:
                            ...
                            logger.warning(f"Worker {gpu}: Verification FAILED (Attempt {attempt}/{args.max_retries})")
                            if args.double_ref_on_failure and attempt == 1: current_ref_text=f"{original_ref_text} {original_ref_text}"; current_ref_audio_chunk=np.concatenate([current_ref_audio_chunk, current_ref_audio_chunk])
                            if args.save_failures:
                                ...
                                debug_gen_audio_path=debug_segment_dir/f"gen_audio_attempt_{attempt}.wav"; os.rename(temp_gen_path,debug_gen_audio_path)
                                info={}
                                if attempt==1:
                                    sf.write(debug_segment_dir/"ref_audio.wav",full_audio_data[start_sample:end_sample],sample_rate)
                ...
                try:
                    ...
                    stats['successful']+=1
                except Exception as e: stats['failed']+=1; failed_segments_info.append({'filename': filename, 'text': original_gen_text})
...
def process_file_pair(args, input_wav_path, input_json_path):
    # ... (Változatlan, a hibajelentő logikával együtt) ...
    ...
    with mp.Manager() as manager:
        ...
        if failed_segments_info:
            ...
            logger.info("The following segments could not be generated successfully:")
