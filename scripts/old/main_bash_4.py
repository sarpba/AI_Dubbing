...
def run_dubbing_workflow(project_name, project_path, config, params, memory_handler):
    ...
    tts_path_run4 = params['tts_model_path_run4'] # Új modell elérési út
    lang_code = params['lang_code']
    
    logging.info(f"--- Dubbing Workflow Indítása: '{project_name}' ---")

    # Alkönyvtárak elérési útjainak meghatározása a configból
    ...
    steps = [
        ([sync_python, 'scripts/extract_audio_easy_channels.py', project_name, '--keep_channels'], "[extract_audio]",  True),
        ([sync_python, 'scripts/separate_audio_easy_channels.py', '-p', project_name], "[separate_audio]", True),
        ...
        ([sync_python, 'scripts/split_segments_by_speaker.py', '-p', project_name], "[split_by_speaker]", True),
        ...
        ([tts_python, 'scripts/f5_tts_easy9_kisérleti.py', project_name, '--phonetic-ref', '--norm', 'hun', '--double-ref-on-failure', '--model_dir', tts_path], "[f5_tts_run1]", False),
        ...
        # --- ÚJ LÉPÉS ---
        ([tts_python, 'scripts/f5_tts_easy9_kisérleti.py', project_name, '--phonetic-ref', '--norm', 'hun', '--save-failures', '--double-ref-on-failure', '--normalize-ref-audio', '--model_dir', tts_path_run4], "[f5_tts_run4]", True),
        # ---
    ...
    for cmd, prefix, is_critical in steps:
        # Ellenőrzések a lépések kihagyásához
        if prefix == "[unpack_srt]":
            try:
                if any(f.lower().endswith('.srt') for f in os.listdir(upload_dir)):
                    logging.info(f"{prefix} .srt fájl már létezik az '{subdirs['upload']}' mappában, a lépés kihagyva.")
                    continue
            except FileNotFoundError:
                pass

        if prefix == "[separate_audio]":
            try:
                if os.path.isdir(speech_dir) and os.path.isdir(background_dir) and os.listdir(speech_dir) and os.listdir(background_dir):
                    logging.info(f"{prefix} Kimeneti audio fájlok már léteznek, a lépés kihagyva.")
                    continue
            except FileNotFoundError:
                pass

        if prefix == "[parakeet_transcribe]":
            try:
                if os.path.isdir(speech_dir) and any(f.lower().endswith('.json') for f in os.listdir(speech_dir)):
                    logging.info(f"{prefix} Kimeneti .json fájl már létezik a '{subdirs['separated_audio_speech']}' mappában, a lépés kihagyva.")
                    continue
            except FileNotFoundError:
                pass

        if prefix == "[split_by_speaker]":
            try:
                if any(f.lower().endswith('.bak') for f in os.listdir(speech_dir)):
                    logging.info(f"{prefix} .bak fájl már létezik a '{subdirs['separated_audio_speech']}' mappában, a lépés kihagyva.")
                    continue
            except FileNotFoundError:
                pass
                
        if prefix == "[translate_chatgpt]":
            try:
                if os.path.isdir(translated_dir) and any(f.lower().endswith('.json') for f in os.listdir(translated_dir)):
                    logging.info(f"{prefix} Kimeneti .json fájl már létezik a '{subdirs['translated']}' mappában, a lépés kihagyva.")
                    continue
            except FileNotFoundError:
                pass
...
def main():
    ...
    if args.auto:
        ...
        # Ellenőrzés kibővítése az új paraméterrel
        if not all(k in params for k in ["translation_context", "tts_model_path", "tts_model_path_run4", "lang_code"]):
            logging.error("Nincsenek elmentett paraméterek az automatikus futáshoz! Futtasd interaktívan (--auto nélkül)."); sys.exit(1)
    else:
        params = {}
    ...
    if args.input_dir:
        ...
        for i, video_path in enumerate(video_files):
            ...
            has_video = any(f.lower().endswith(VIDEO_EXTENSIONS) for f in os.listdir(upload_dir))
            
            if not has_video:
                logging.info(f"Videófájl másolása a projektbe: {project_name}")
                shutil.copy2(video_path, upload_dir)
            else:
                logging.info(f"Videófájl már létezik a projektben, a másolás kihagyva.")

            memory_handler, file_handler = setup_logging(project_path, config['PROJECT_SUBDIRS']['logs'], project_name)
            try:
                if run_dubbing_workflow(project_name, project_path, config, params, memory_handler):
        ...
        try:
            ...
            if not args.auto:
                ...
                # Modellválasztás a 4. futtatáshoz
                last_model_name_run4 = os.path.basename(last_params.get("tts_model_path_run4", ""))
                # Az alapértelmezett a run1-3-hoz választott modell lesz
                selected_model_run4 = select_from_list(available_models, "\nTTS modell a 4. futtatáshoz (run4):", default_value=selected_model)
                if not selected_model_run4: logging.error("TTS modell választása a 4. futtatáshoz kötelező!"); sys.exit(1)
