    ...
    try:
        ...
        if args.seed == -1:
            if num2words is None:
                logger.error("Verification requires 'num2words'. Please 'pip install num2words'."); return
        ...
        # --- JAVÍTOTT, SZINTAKTIKAILAG HELYES BLOKK ---
        normalize_fn = None
        ...
        if normaliser_path.exists():
            try:
                ...
                if hasattr(normaliser, 'normalize'):
                    ...
                    logger.warning(f"Normaliser file '{normaliser_path}' exists but has no 'normalize' function.")
            except Exception as e:
                logger.error(f"Worker {gpu}: Failed to load or process normalizer '{args.norm}': {e}.")
        else:
            logger.info(f"Worker {gpu}: Normalizer module not found at '{normaliser_path}'. Proceeding without custom normalization.")
        # -----------------------------------------------
        ...
        for segment in tqdm.tqdm(tasks_chunk, desc=f"Processing on {device}", position=gpu):
            ...
            original_ref_text=segment.get('text','').strip(); gen_text=segment.get('translated_text','').strip()
            ...
            if args.seed == -1:
                ...
                try:
                    ...
                    stats['successful']+=1
                except Exception as e: logger.error(f"Worker {gpu}: Direct generation FAILED: {e}", exc_info=False); stats['failed']+=1
