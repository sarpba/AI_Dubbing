...
def upload_and_extract_audio(proj_name, video_path, workdir="workdir"):
    ...
    try:
        ...
        # Feliratok kinyerése
        # Először ellenőrizzük, hogy van-e felirat stream a videóban
        ffprobe_cmd = [
            "ffprobe",
            "-v", "error",
            "-select_streams", "s",
        ...
        if not subtitle_streams or (len(subtitle_streams) == 1 and subtitle_streams[0] == ''):
            ...
            for stream_info in subtitle_streams:
                if stream_info:
                    ...
                    language = parts[1] if len(parts) > 1 and parts[1] else 'unknown'
