...
Az `main_app.py` Flask-alapú vezérlőfelület a `scripts/` alatti modulokból automatikusan épített script-katalógus alapján kínálja fel a futtatható lépéseket, így a teljes workflow egy helyről indítható.
...
- `scripts/How_to_create_new_script_modul/how_to_create_script_modul.md` – Embernek szóló, összefoglalt útmutató új script modulokhoz: `.py` + `.json` + `_help.md`, CLI/JSON szinkron, help fájl és alap fejlesztési checklista.
- `scripts/How_to_create_new_script_modul/how_to_create_script_modul_AI.md` – Repo-specifikus belső leírás a tényleges integrációs szabályokról: `environment`, automatikus `scripts/scripts.json` újraépítés, flag logika, keyholder és workflow UI kompatibilitás.

### Script modul fejlesztés röviden
- Minden új modul három fájlból álljon: `<name>.py`, `<name>.json`, `<name>_help.md`.
- Az új JSON fájlokban `environment` kulcsot használj, ne `enviroment`.
- A `scripts/scripts.json` derivált fájl; kézzel ne szerkeszd, a rendszer újraépíti a modulok saját JSON leíróiból.
- A JSON, az `argparse` és a `_help.md` paraméterlistája maradjon 1:1-ben szinkronban.
- Minden paraméter kapjon explicit `default` mezőt.
- Ha a script API kulcsot vagy speciális autofillt használ, ellenőrizd a kapcsolódó backend mappingeket is.
